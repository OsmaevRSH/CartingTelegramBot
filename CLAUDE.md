...
utils/                         # health_check.py, clear_database.py
...
- `/stats` — статистика пользователей; поддерживает возврат к выбору пользователя кнопкой в интерфейсе
- `/best` — глобальная таблица лучших кругов
- `/best_today` — таблица лучших за сегодня
- `/cancel` — отменить текущий диалог `/add` (команда работает, но скрыта из меню команд)
...
Единственный `.env` файл находится в корне проекта. Копировать `env.example` в `.env`.
...
Docker читает тот же `.env` через `env_file: ../.env` в `docker-compose.yml`, но переопределяет
`DATABASE_PATH=/app/data/races.db` и `LOG_FILE=/app/logs/bot.log` под пути внутри контейнера.
...
- `stats_back_to_users_callback` — возврат к выбору пользователя в `/stats` (паттерн `^stats_back_users$`)
